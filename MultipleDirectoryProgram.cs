...
namespace Shockpile
{
    class MultipleDirectoryProgram
    {
        ...
        private static void DoVersion(int version)
        {
            string node = @"C:\Program Files\nodejs\node.exe";
            string projectraysDirectory = @"C:\Users\Alex\Documents\GitHub\ProjectorRays";
            string cstDirectory = @"C:\Users\Alex\Documents\GitHub\habbo_src\v" + version + "\\src";
            string srcDirectory = cstDirectory;//Path.Combine(cstDirectory, "src");//@"C:\Users\Alex\Documents\Decompiler\v7\src";

            if (!Directory.Exists(projectraysDirectory))
            {
                WriteError("The projectrays directory is not supplied!");
                return;
            }

            if (!Directory.Exists(cstDirectory))
            {
                WriteError("The directory for cst files doesn't exist!");
                return;
            }

            if (!Directory.Exists(srcDirectory))
            {
                WriteWarning("The src directory doesn't exist... creating");
            }
        ...
        private static void WriteError(string v)
        {
